...
function RegistroContenido() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: todasMascotas } = await supabase.from('mascotas').select('id,nombre,especie,raza,foto_url,sexo,castrado,seguimiento_reproductivo,fecha_nacimiento').is('archivada_en', null).order('created_at', { ascending: true })
