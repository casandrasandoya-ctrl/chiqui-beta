...
// Valores de referencia según cardiología veterinaria (iguales para perros y gatos)
// La única diferencia es el texto de urgencia — en gatos el jadeo es emergencia crítica
const RANGOS_BASE = [
  { min: 0,   max: 15,  label: 'Muy baja', color: '#4AABDB', bg: '#EBF6FC',
    texto: (_: string) => 'Frecuencia inusualmente baja. Consulta con tu vet.' },
  { min: 15,  max: 30,  label: 'Normal',   color: '#4CAF7D', bg: '#EAF6EF',
    texto: (_: string) => 'Frecuencia normal en reposo. El movimiento debe ser suave.' },
  { min: 30,  max: 40,  label: 'Atención', color: '#F5C842', bg: '#FEF9E7',
    texto: (_: string) => 'Mantente alerta. Monitorea varias veces al día. Si persiste, consulta al vet.' },
  { min: 40,  max: 999, label: 'Urgente',  color: '#E05252', bg: '#FDEAEA',
    texto: (esp: string) => esp === 'Gato'
      ? 'Acude a urgencias de inmediato. Los gatos NUNCA deben jadear con la boca abierta — es señal crítica de falta de oxígeno o falla cardíaca avanzada.'
      : 'Visita al veterinario de inmediato.' },
]

function getRango(rpm: number, especie: string) {
  const rango = RANGOS_BASE.find(r => rpm >= r.min && rpm < r.max) || RANGOS_BASE[1]
...
export default function RespiracionTracker({ mascotaId, especie }: Props) {
  ...
  // Solo aplica a perros y gatos
  if (especie !== 'Perro' && especie !== 'Gato') return null

  const RANGOS = RANGOS_BASE
    ...
    <div className="border-t border-[#EEE2D4] pt-3">
      ...
      {especie === 'Gato' && (
        <div className="bg-[#FDEAEA] border border-[#E05252]/20 rounded-xl px-3 py-2 mb-3 flex gap-2">
          ...
          <p className="text-xs text-[#E05252] leading-relaxed">
            <strong>En gatos, jadear con la boca abierta es siempre una emergencia.</strong> A diferencia de los perros, los gatos nunca jadean por calor o cansancio normal — es señal crítica de falta de oxígeno.
          </p>
        </div>
      )}
      ...
      {mostrarInstrucciones && (
        <div className="bg-[#FBEAD9] rounded-xl px-3 py-3 mb-3 space-y-2">
          ...
          {[
            `1. Espera que tu ${especie === 'Gato' ? 'gato' : 'perro'} esté completamente dormido o en reposo total (no soñando).`,
            ...
            {RANGOS.slice(1).map(r => (
              <div key={r.label} className="flex items-center gap-2">
                ...
                <p className="text-xs text-[#7A4A2F]">
                  <strong>{r.label}:</strong> {r.min === 40 ? '+40' : `${r.min}-${r.max}`} rpm — {r.texto(especie)}
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#FFFCF8] border border-[#EEE2D4] rounded-xl p-3 mb-3 space-y-2">
              ...
              return (
                <div className="flex-shrink-0 text-center">
                  <div className="w-14 h-14 rounded-xl flex items-center justify-center" style={{ background: r.bg }}>
                    <span className="text-xs font-bold text-center px-1" style={{ color: r.color }}>{r.label}</span>
                  </div>
                </div>
              )
            })()}
          ...
          {rpm && parseInt(rpm) > 0 && (
            <p className="text-xs leading-relaxed" style={{ color: getRango(parseInt(rpm), especie).color }}>
              {getRango(parseInt(rpm), especie).textoFinal}
            </p>
          )}
          <input
            type="text"
