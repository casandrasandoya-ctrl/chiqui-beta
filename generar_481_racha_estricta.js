...
// hitoRacha vive en Novedades y tiene los 15 niveles de imagenes ya
// diseñados. Se exporta para que el modal use LA MISMA, en vez de
// duplicar una lista que se desincronizaria al primer cambio.
if (!fs.existsSync(dNov)) {
  abortar('no se encontro ' + RUTA_NOV + '.');
}
let nov = fs.readFileSync(dNov, 'utf8');
const EXPORT_VIEJO = "function hitoRacha(dias: number): { img: string; mensaje: string } {";
const EXPORT_NUEVO = "// Se exporta para que el modal de racha use LA MISMA funci\u00f3n: dos\n// sistemas de im\u00e1genes por nivel se desincronizan al primer cambio.\nexport function hitoRacha(dias: number): { img: string; mensaje: string } {";
if (!nov.includes('export function hitoRacha')) {
  const nE = contar(nov, EXPORT_VIEJO);
  console.log('  ' + (nE === 1 ? 'OK ' : 'X  ') + 'hitoRacha en Novedades -> ' + nE + ' coincidencia(s)');
  if (nE !== 1) {
    abortar('esperaba 1 coincidencia de hitoRacha y encontre ' + nE + '.');
  }
...
// El script 478 alcanzo a crear este archivo antes de que el build
// fallara, asi que ya existe con la version rota. Se sobrescribe, pero
// SOLO si es reconociblemente el mio: si alguien lo edito a mano, mejor
// abortar y avisar.
if (fs.existsSync(dModal)) {
  const previo = fs.readFileSync(dModal, 'utf8');
  if (!previo.includes('export default function ModalLogro')) {
    abortar('ya existe ' + RUTA_MODAL + ' y no lo reconozco. No lo sobrescribo.');
  }
  console.log('  --  ya existia el modal (version anterior): se reemplaza');
}
...
// Si el 478 alcanzo a modificar el registro diario, hay que revertirlo
// antes de aplicar la version buena: quedarian dos modales.
if (reg.includes('ModalLogro')) {
  abortar('el registro diario ya tiene el modal. Si el build fallo con la version anterior, avisale a Claude: hay que revertir esa parte primero.');
}
...
console.log('');
console.log('El modal usa las MISMAS imagenes que Novedades: chiqui_racha_7,');
console.log('chiqui_racha_15, chiqui_racha_30, chiqui_racha_100, las coronas y');
console.log('el superheroe. Una sola lista, sin duplicar.');
console.log('');
