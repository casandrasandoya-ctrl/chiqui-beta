...
const PARES = [
  { nombre: 'estado del logro', viejo: "  const [confirmarGuardado, setConfirmarGuardado] = useState(false)", nuevo: "  const [confirmarGuardado, setConfirmarGuardado] = useState(false)\n  // Datos del modal de logro. null mientras no se ha guardado.\n  const [logro, setLogro] = useState<{ racha: number; mejorRacha: number; ultimos7: { letra: string; hecho: boolean }[]; diasDelMes: number; diasMesPasado: number } | null>(null)" },
  { nombre: 'calculo tras guardar', viejo: "    router.push('/dashboard')\n    router.refresh()\n  }", nuevo: "    // El modal de logro: se calcula la racha y se muestra ANTES de\n    // volver al dashboard. Este es el momento en que la persona ya hizo\n    // el esfuerzo, y reconocerlo ac\u00e1 es lo que hace que vuelva ma\u00f1ana.\n    try {\n      const hoyL = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santiago' }).format(new Date())\n      const haceUnA\u00f1o = new Date(hoyL + 'T12:00:00')\n      haceUnA\u00f1o.setDate(haceUnA\u00f1o.getDate() - 400)\n      const desdeL = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santiago' }).format(haceUnA\u00f1o)\n\n      const { data: todos } = await supabase\n        .from('registros_diarios')\n        .select('fecha')\n        .eq('mascota_id', mascotaId)\n        .gte('fecha', desdeL)\n        .order('fecha', { ascending: false })\n\n      const fechas = new Set((todos || []).map((r: any) => String(r.fecha).slice(0, 10)))\n      // La fecha que se acaba de guardar puede no venir en la consulta\n      // todav\u00eda, as\u00ed que se agrega a mano.\n      if (fechaRegistro) fechas.add(fechaRegistro)\n\n      // La racha: d\u00edas consecutivos hacia atr\u00e1s desde hoy. Mediod\u00eda para\n      // que los cambios de horario de verano no la corten.\n      let r = 0\n      const cursor = new Date(hoyL + 'T12:00:00')\n      while (true) {\n        const f = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santiago' }).format(cursor)\n        if (!fechas.has(f)) break\n        r++\n        cursor.setDate(cursor.getDate() - 1)\n      }\n\n      // Los \u00faltimos 7 d\u00edas TERMINANDO HOY, con el nombre real de cada\n      // uno. Empezar en domingo dejaba casi todos los checks fuera de\n      // vista los lunes, que es cuando menos se ve el esfuerzo.\n      const DIAS = ['dom', 'lun', 'mar', 'mi\u00e9', 'jue', 'vie', 's\u00e1b']\n      const ult7: { letra: string; hecho: boolean }[] = []\n      for (let i = 6; i >= 0; i--) {\n        const dia = new Date(hoyL + 'T12:00:00')\n        dia.setDate(dia.getDate() - i)\n        const f = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santiago' }).format(dia)\n        ult7.push({ letra: DIAS[dia.getDay()], hecho: fechas.has(f) })\n      }\n\n      // La MEJOR racha hist\u00f3rica: la secuencia m\u00e1s larga de d\u00edas\n      // seguidos que haya tenido. Ver cu\u00e1nto falta para superarla\n      // motiva m\u00e1s que el n\u00famero actual solo.\n      const ordenadas = Array.from(fechas).sort()\n      let mejor = 0\n      let seguidos = 0\n      let anterior: string | null = null\n      for (const f of ordenadas) {\n        if (anterior) {\n          const a = new Date(anterior + 'T12:00:00').getTime()\n          const b = new Date(f + 'T12:00:00').getTime()\n          seguidos = Math.round((b - a) / 86400000) === 1 ? seguidos + 1 : 1\n        } else {\n          seguidos = 1\n        }\n        if (seguidos > mejor) mejor = seguidos\n        anterior = f\n      }\n\n      const mesActual = hoyL.slice(0, 7)\n      const mesAnterior = (() => {\n        const d = new Date(hoyL + 'T12:00:00')\n        d.setMonth(d.getMonth() - 1)\n        return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santiago' }).format(d).slice(0, 7)\n      })()\n      const delMes = Array.from(fechas).filter(f => f.startsWith(mesActual)).length\n      const delAnterior = Array.from(fechas).filter(f => f.startsWith(mesAnterior)).length\n\n      setLogro({ racha: r, mejorRacha: mejor, ultimos7: ult7, diasDelMes: delMes, diasMesPasado: delAnterior })\n      setLoading(false)\n      return\n    } catch {\n      // Si el c\u00e1lculo falla, se vuelve al dashboard como siempre: el\n      // registro ya se guard\u00f3, que es lo que importa.\n    }\n\n    router.push('/dashboard')\n    router.refresh()\n  }" },
  { nombre: 'render del modal', viejo: "    <div className=\"min-h-screen pb-24 fade-in\">\n      {/* Modal tras \"Todo normal\": guardar al toque o seguir editando */}", nuevo: "    <div className=\"min-h-screen pb-24 fade-in\">\n      {/* El logro tras guardar. Va primero porque cubre toda la\n          pantalla. */}\n      {logro && (\n        <ModalLogro\n          nombre={mascotaNombre}\n          racha={logro.racha}\n          mejorRacha={logro.mejorRacha}\n          ultimos7={logro.ultimos7}\n          diasDelMes={logro.diasDelMes}\n          diasMesPasado={logro.diasMesPasado}\n          editando={yaRegistro}\n          onCerrar={() => { router.push('/dashboard'); router.refresh() }}\n        />\n      )}\n      {/* Modal tras \"Todo normal\": guardar al toque o seguir editando */}" },
];
